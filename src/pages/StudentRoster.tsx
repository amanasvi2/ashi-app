    ...
    <div className="min-h-screen bg-slate-50">
      <div className="max-w-4xl mx-auto px-4 md:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest text-slate-400 mb-0.5">Caseload</p>
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">{rows.length} {rows.length === 1 ? 'student' : 'students'}</h1>
          </div>
          <button onClick={onLogout} className="text-xs text-slate-400 hover:text-slate-600 transition-colors mt-1">
            Log out
          </button>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200/70 shadow-[0_1px_2px_rgba(0,0,0,0.03)] divide-y divide-slate-50">
          {rows.map(row => (
            <button
              key={row.student.id}
              onClick={() => onSelectStudent(row.student.id)}
              className="w-full flex items-center gap-4 px-5 py-4 text-left hover:bg-blue-50/30 transition-colors"
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <p className="text-sm font-semibold text-slate-800 capitalize">{row.student.displayName}</p>
                  {row.floorAlarm && (
                    <span className="text-xs font-semibold text-rose-700 bg-rose-50 px-2 py-0.5 rounded-full shrink-0">
                      Worth a look
                    </span>
                  )}
                </div>
                <p className="text-xs text-slate-400 mt-0.5">Last session: {formatDate(row.lastSessionDate)}</p>
              </div>
              ...
              <div className="hidden sm:flex gap-2 shrink-0">
                {ITEM_TYPES.map(t => (
                  <span
                    ...
                    className={`text-[11px] font-medium px-2 py-1 rounded-full ${LEVEL_COLORS[row.levels[t]]}`}
                  >
          ...
          {rows.length === 0 && (
            <p className="text-center text-sm text-slate-400 py-8">No students yet.</p>
          )}
        </div>
