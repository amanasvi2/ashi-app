...
export function ProfilePanel({ username, onClose, onLogout }: Props) {
  ...
  const update = (next: Partial<ParentConfig>) => {
  ...
  const toggleInterest = (id: string) => {
    const interests = config.interests.includes(id)
      ? config.interests.filter(x => x !== id)
      : [...config.interests, id];
    update({ interests });
  };
          ...
          <div>
            ...
            <div className="flex gap-2">
              {([
                ...
                <button
                  key={opt.value}
                  onClick={() => update({ kidGender: opt.value })}
                  className={`flex-1 py-2 rounded-xl text-sm border transition-all
                    ${config.kidGender === opt.value
              ...
              {INTEREST_OPTIONS.map(opt => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-sm border transition-all
                    ${config.interests.includes(opt.id)
